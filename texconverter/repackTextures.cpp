...

	{
		...
		for (auto i = 0; i < entries.size(); ++i)
		{
			...
			if (outputs[OutputDXT])
			{
				...
				KtxHeader header;
				header.internal_format = channels % 2 == 0 ? GL_COMPRESSED_RGBA_S3TC_DXT5_EXT : GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
				header.base_internal_format = channels % 2 == 0 ? GL_RGBA : GL_RGB;
				header.pixel_width = width;
				header.pixel_height = height;
				fwrite(&header, sizeof(header), 1, dst_pack.get());
				auto image_length = static_cast<uint32_t>(compressed_size);
				fwrite(&image_length, sizeof(image_length), 1, dst_pack.get());
				fwrite(compressed.data(), compressed_size, 1, dst_pack.get());

				// Update entry information (replace extension, fixup size, position)
				auto position = static_cast<int32_t>(base_offsets[OutputDXT]);
				auto size = static_cast<int32_t>(compressed_size + sizeof(header) + sizeof(uint32_t));
			...
			if (outputs[OutputASTC])
			{
				...
				KtxHeader header;
				header.internal_format = GL_COMPRESSED_RGBA_ASTC_4x4_KHR;
				header.base_internal_format = GL_RGBA;
				header.pixel_width = width;
				header.pixel_height = height;
				fwrite(&header, sizeof(header), 1, dst_pack.get());
				auto image_length = static_cast<uint32_t>(compressed_size);
...
int process_image(std::string_view src_name, const file_ptr& src_file, std::bitset<OutputCount> outputs)
{
	...
	if (outputs[OutputDXT])
	{
		...
		fwrite(&header, sizeof(header), 1, dxt_ktx.get());
		
		auto image_length = static_cast<uint32_t>(compressed_size);
		fwrite(&image_length, sizeof(image_length), 1, dxt_ktx.get());
		fwrite(compressed.data(), compressed_size, 1, dxt_ktx.get());
	}
	...
	if (outputs[OutputASTC])
	{
		...
		fwrite(&header, sizeof(header), 1, astc_ktx.get());

		auto image_length = static_cast<uint32_t>(compressed_size);
		fwrite(&image_length, sizeof(image_length), 1, astc_ktx.get());
		fwrite(compressed.data(), compressed_size, 1, astc_ktx.get());
		astcenc_compress_reset(context.get());
	}
